...
class LMModel(StreamingModule):
    ...
    def forward(self, sequence: torch.Tensor,
        ...
        # print("condition_tensors : ", condition_tensors)
    ...
    def generate(self,
        ...
        if conditions:
            ...
            if two_step_cfg:
                ...
                # print("conditions : ", conditions[0].text['description'])
                descs = parse_text_with_weights(conditions[0].text['description'])
                # print("desc : ", descs)
                index_count = 0
                for desc in descs:
                    value = desc['value']
                    weight = desc['weight']
                    vt = [ConditioningAttributes(text={'description': value})]
                    token = self.condition_provider.tokenize(vt)
                    token_len = len(token['description']['input_ids'][0][:-1])
                    if weight != 1.0:
                        weight_index.append([index_count, index_count+token_len, weight])
                    index_count += token_len
        ...
        # 마지막 토큰이 eos token
        # print("\n\ncfg_conditions.shape : ", cfg_conditions['description'][0].shape, "\n\n", cfg_conditions['description'][0], "\n\n")
        # cfg_conditions['description'][0].shape : (cond+null=2, tokens_num, codebooksize=1536)
        for w in weight_index:
            start = w[0]
            end = w[1]
            weight = w[2]
            # print("start, end, weight : ", start, end, weight)
            cfg_conditions['description'][0][0][start:end] *= weight
